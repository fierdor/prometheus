...
while($row1 = mysql_fetch_array($res2) )
{
	...
	imageline($image,coord($row1['id'],"x"),coord($row1['id'],"y"),coord($row2['id'],"x"),coord($row2['id'],"y"),$blue);
}
...
if($row['state']=="up")
imagefilledellipse($image, $row['x'], $row['y'], 20, 20, $blue);
